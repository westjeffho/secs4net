...
namespace Secs4Net {
    public sealed class SecsGem : IDisposable {
            ...
            if (msg.ReplyExpected) {
                ...
                ThreadPool.RegisterWaitForSingleObject(ar.AsyncWaitHandle,
                   (state, timeout) => {
                       SecsAsyncResult ars;
                       if (_replyExpectedMsgs.TryRemove((int)state, out ars) && timeout) {
                           _tracer.TraceError($"T3 Timeout[id=0x{ars.SystemByte:X8}]");
        ...
        public SecsMessage Send(SecsMessage msg) => this.EndSend(this.BeginSend(msg, null, null));
        ...
        /// <param name="msg"></param>
        /// <returns></returns>
        public async Task<SecsMessage> SendAsync(SecsMessage msg) => await Task.Factory.FromAsync<SecsMessage>(BeginSend(msg, null, null), EndSend);
        ...
        public IAsyncResult BeginSend(SecsMessage msg, AsyncCallback callback, object state) => this.SendDataMessage(msg, NewSystemByte(), callback, state);
        ...
        public SecsMessage EndSend(IAsyncResult asyncResult) {
            ...
            if (ar == null)
                throw new ArgumentException("參數asyncResult不是BeginSend所取得的", nameof(asyncResult));
        ...
        public string DeviceAddress => _isActive ? _ip.ToString() :
